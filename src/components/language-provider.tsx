"use client";

import { createContext, useContext, useEffect, useMemo, useState } from "react";

export type UiLanguage = "tr" | "en";

const translations = {
  tr: {
    dashboard: "Ana Sayfa",
    reportIntelligence: "Rapor Zekâsı",
    reports: "Raporlar",
    knowledgeBase: "Bilgi Bankası",
    seniorConsultant: "Kıdemli Danışman",
    workspaceSettings: "Çalışma alanı ayarları",
    notifications: "Bildirimler",
    aiOperational: "Yapay zekâ servisleri çalışıyor",
    reportWorkspace: "Rapor çalışma alanı",
    greeting: "Günaydın",
    dashboardDescription: "Ekibinizin bilgisiyle güvenilir ve kaynaklara dayalı raporlar hazırlayın.",
    newReport: "Yeni rapor",
    allReports: "Tüm raporlar",
    inProgress: "Devam ediyor",
    needsReview: "İnceleme gerekli",
    completed: "Tamamlandı",
    recentReports: "Son raporlar",
    recentReportsDescription: "Düzenlemeye devam edin veya oluşturulan içeriği inceleyin.",
    searchReports: "Raporlarda ara...",
    loadingReports: "Raporlar yükleniyor...",
    progress: "İlerleme",
    noReports: "Henüz rapor yok. Başlamak için ilk raporunuzu oluşturun.",
    backToDashboard: "Ana sayfaya dön",
    newReportEyebrow: "Yeni rapor",
    newReportTitle: "Rapor çalışma alanınızı oluşturun",
    newReportDescription: "Proje bilgilerini girin. Kaynakları ve belgeleri daha sonra da ekleyebilirsiniz.",
    reportDetails: "Rapor bilgileri",
    reportType: "Rapor türü",
    projectName: "Proje adı",
    projectPlaceholder: "örn. Kuzey Bölgesi Gelişim Projesi",
    locationParcel: "Konum ve parsel",
    city: "İl",
    district: "İlçe",
    neighborhood: "Mahalle",
    parcelInfo: "Ada / parsel bilgileri",
    parcelPlaceholder: "Ada 123, Parsel 45",
    aiSettings: "AI Ayarları",
    aiSettingsDescription: "Açık olduğunda AI tanımlı rapor kaynaklarını ve güvenilir internet kaynaklarını kullanabilir.",
    outputPreferences: "Çıktı tercihleri",
    outputLanguage: "Rapor çıktı dili",
    desiredLength: "Hedef rapor uzunluğu",
    pagesApprox: "Yaklaşık {count} sayfa",
    createReport: "Rapor oluştur",
    creatingWorkspace: "Çalışma alanı oluşturuluyor...",
    whatNext: "Sonraki adımlar",
    nextTemplate: "10 bölümlü profesyonel rapor şablonu oluşturulur.",
    nextConfiguredSources: "Rapor türü için tanımlı kaynaklar otomatik olarak alınır.",
    nextFiles: "PDF, DOCX veya TXT kaynak dosyaları yükleyebilirsiniz.",
    nextGenerate: "Her bölüm kaynaklarla oluşturulur ve incelemeye sunulur.",
    manualReviewNote: "Yeterli kanıtı olmayan yapay zekâ iddiaları otomatik olarak manuel incelemeye işaretlenir.",
    planningReport: "Planlama ve Geliştirme Raporu",
    feasibilityReport: "Fizibilite Raporu",
    dueDiligenceReport: "Durum Tespit Raporu",
    marketAssessment: "Pazar Değerlendirmesi",
    turkish: "Türkçe",
    english: "İngilizce",
    german: "Almanca",
    save: "Kaydet",
    saving: "Kaydediliyor...",
    allChangesLocal: "Tüm değişiklikler yerel",
    exportDocx: "DOCX dışa aktar",
    reportProgress: "RAPOR İLERLEMESİ",
    sectionsStarted: "{completed} / {total} bölüm başlatıldı",
    reportSections: "Rapor bölümleri",
    section: "Bölüm",
    confidence: "güven",
    contentEditor: "İçerik düzenleyici",
    words: "kelime",
    regenerateSection: "Bölümü yeniden oluştur",
    generateSection: "Bölümü oluştur",
    editorPlaceholder: "Bu bölümü yapay zekâ ile oluşturun veya yazmaya başlayın...",
    evidenceChecks: "Kanıt kontrolleri etkin",
    markApproved: "Onaylandı olarak işaretle",
    assistant: "Asistan",
    sources: "Kaynaklar",
    review: "İnceleme",
    quickActions: "Hızlı işlemler",
    formal: "Daha resmî yaz",
    expandEvidence: "Kanıtlarla genişlet",
    findMissing: "Eksik bilgileri bul",
    showUnsupported: "Desteksiz iddiaları göster",
    convertTable: "Tabloya dönüştür",
    officialOnly: "Yalnızca resmî kaynakları kullan",
    assistantReady: "Bu bölümde yardımcı olmaya hazırım. Rapor bağlamını kullanacak ve desteksiz iddiaları görünür tutacağım.",
    applyRevision: "Düzenlemeyi uygula",
    reviewingContext: "Bağlam inceleniyor...",
    askSection: "Bu bölüm hakkında sorun...",
    evidenceLibrary: "Kanıt kütüphanesi",
    evidenceDescription: "Yapay zekâ üretiminde kullanılabilecek internet kaynakları ve yüklenen dosyalar.",
    uploadFiles: "PDF, DOCX veya TXT yükle",
    extractingText: "Metin çıkarılıyor...",
    officialWebSource: "Resmî internet kaynağı",
    webSource: "İnternet kaynağı",
    configuredSources: "Tanımlı kaynaklar",
    manualSources: "Manuel eklenen kaynaklar",
    aiDiscoveredSources: "AI tarafından keşfedilen kaynaklar",
    aiDiscoveredSource: "AI keşfetti",
    configuredSource: "Tanımlı kaynak",
    manualSource: "Manuel kaynak",
    searchQuery: "Arama sorgusu",
    indexedChunks: "{count} indekslenmiş parça",
    noEvidence: "Henüz kanıt eklenmedi.",
    unsupportedClaims: "Desteksiz iddialar",
    missingInformation: "Eksik bilgiler",
    usedSources: "Kullanılan kaynaklar",
    noWarnings: "Bu bölüm için etkin doğrulama uyarısı yok.",
    loadingWorkspace: "Rapor çalışma alanı yükleniyor...",
    draft: "Taslak",
    notStarted: "Başlanmadı",
    generated: "Oluşturuldu",
    approved: "Onaylandı",
    high: "Yüksek",
    medium: "Orta",
    low: "Düşük",
    allowWebResearch: "AI internet araştırması yapabilsin",
    allowWebResearchHelp: "Açık olduğunda AI tanımlı rapor kaynaklarını ve güvenilir internet kaynaklarını kullanabilir.",
    settings: "Ayarlar",
    profile: "Profil",
    workspaceAccount: "Çalışma Alanı Hesabı",
    profileDescription: "Kişisel bilgilerinizi görün, kullanıcı adınızı güncelleyin ve giriş şifrenizi değiştirin.",
    profileLoading: "Profil yükleniyor...",
    personalInformation: "Kişisel Bilgiler",
    personalInformationDescription: "Uygulamada görünen ad ve iletişim bilgileri.",
    security: "Güvenlik",
    securityDescription: "Mevcut şifrenizi doğrulayarak giriş şifresini güncelleyin.",
    fullName: "Ad soyad",
    title: "Unvan",
    email: "E-posta",
    phone: "Telefon",
    username: "Kullanıcı adı",
    updated: "Güncellendi",
    currentPassword: "Mevcut şifre",
    newPassword: "Yeni şifre",
    confirmNewPassword: "Yeni şifre tekrar",
    saveProfile: "Profili kaydet",
    savingProfile: "Kaydediliyor...",
    updatePassword: "Şifreyi güncelle",
    updatingPassword: "Güncelleniyor...",
    profileUpdated: "Profil bilgileri güncellendi.",
    passwordUpdated: "Giriş şifresi güncellendi.",
    signOut: "Çıkış yap",
    signingOut: "Çıkış yapılıyor...",
    signOutError: "Çıkış yapılamadı.",
    secureWorkspace: "Güvenli Çalışma Alanı",
    authentication: "Kimlik doğrulama",
    signIn: "Giriş yap",
    signingIn: "Giriş yapılıyor...",
    loginTitle: "Rapor çalışma alanına erişmek için oturum açın.",
    loginDescription: "Yapılandırılmış API ve sayfa rotaları artık doğrulanmış bir oturum gerektiriyor.",
    loginWorkspaceDescription: "Bu ilk adım, uygulama ve API yüzeyini yetkisiz erişime kapatır. Sonraki aşamada kullanıcı bazlı yetkilendirme ve rapor sahipliği eklenebilir.",
    loginFailed: "Giriş başarısız.",
    loginRequestFailed: "Giriş isteği tamamlanamadı.",
    invalidUsernameOrPassword: "Kullanıcı adı veya şifre hatalı.",
    profileLoadError: "Profil yüklenemedi.",
    profileUpdateError: "Profil güncellenemedi.",
    passwordUpdateError: "Şifre güncellenemedi.",
    passwordRuleMinLength: "Yeni şifre en az 8 karakter olmalı.",
    passwordRuleMatch: "Yeni şifre ve tekrar alanı aynı olmalı.",
    passwordRuleCurrent: "Mevcut şifre doğru girilmeli.",
    invalidRequestData: "Gönderilen bilgiler geçersiz.",
    passwordConfirmationMismatch: "Yeni şifre tekrarı eşleşmiyor.",
    currentPasswordIncorrect: "Mevcut şifre yanlış.",
    fieldRequired: "Bu alan zorunludur.",
    sourceSettings: "Rapor kaynakları",
    templateSettings: "Rapor Şablonları",
    reportTemplates: "Rapor Şablonları",
    reportTemplatesTitle: "Rapor Şablonları",
    reportTemplatesDescription: "Rapor türlerini, bölümleri ve varsayılan kaynakları merkezi olarak yönetin.",
    createReportType: "Rapor türü oluştur",
    importTemplateFromDocument: "Dokümandan şablon oluştur",
    importingTemplate: "Şablon çıkarılıyor...",
    templateImported: "Şablon taslağı hazırlandı.",
    templateImportError: "Dokümandan şablon çıkarılamadı.",
    importedTemplateDraft: "İçe aktarılan taslak",
    importedTemplateNeedsSave: "Bu şablon henüz kaydedilmedi. Düzenleyip kaydedin.",
    deleteReportType: "Rapor türünü sil",
    noReportTypes: "Henüz rapor türü yok. Yeni bir şablon oluşturarak başlayın.",
    generalSettings: "Genel Ayarlar",
    sectionsTab: "Bölümler",
    sourcesTab: "Kaynaklar",
    aiConfigurationTab: "AI Configuration",
    generalTab: "Genel Ayarlar",
    sectionTitle: "Bölüm başlığı",
    sectionDescription: "Bölüm açıklaması",
    sectionAiPromptLabel: "AI Prompt",
    sectionAiPromptPlaceholder: "Konumu resmî kamu kaynaklarını kullanarak açıkla.\nİl, ilçe, komşu yerleşimler ve bölgesel özellikleri anlat.\nResmî planlama dilinde yaz.",
    sectionRequiredLabel: "Zorunlu",
    sectionEnabledLabel: "Etkin",
    aiConfigGeneralCardTitle: "Genel",
    aiConfigPromptCardTitle: "AI Prompt",
    aiConfigOutputCardTitle: "Çıktı",
    aiConfigCitationsCardTitle: "Atıflar",
    aiConfigReportLanguageLabel: "Rapor dili",
    aiConfigEnableWebResearchLabel: "Web araştırmasını etkinleştir",
    aiConfigDefaultPromptLabel: "Varsayılan AI Prompt",
    aiConfigDefaultPromptPlaceholder: "Profesyonel bir imar raporu oluştur.\nResmî terminoloji kullan.\nBilgi uydurma.\nMümkün olduğunda güvenilir kamu kaynaklarını kullan.",
    aiConfigCreativityLabel: "Yaratıcılık seviyesi",
    aiConfigRequireCitationsLabel: "Atıf zorunlu",
    aiConfigReportToneLabel: "Rapor tonu",
    aiConfigDocumentFormatLabel: "Belge formatı",
    aiConfigToneFormal: "Resmî",
    aiConfigToneTechnical: "Teknik",
    aiConfigToneLegal: "Hukuki",
    aiConfigToneAcademic: "Akademik",
    addSection: "Bölüm ekle",
    noSections: "Bu rapor türü için henüz bölüm tanımlanmadı.",
    sourceName: "Kaynak adı",
    sourceDescription: "Kaynak açıklaması",
    addSource: "Kaynak ekle",
    noTemplateSources: "Bu rapor türü için henüz varsayılan kaynak tanımlanmadı.",
    createTemplatePlaceholder: "örn. İmar Planı Açıklama Raporu",
    templateCreateHint: "İsim girerseniz o adla oluşturulur. Boş bırakırsanız otomatik bir taslak şablon oluşturulur.",
    templateDescriptionPlaceholder: "Bu rapor türünün ne amaçla kullanıldığını kısa şekilde açıklayın.",
    saveTemplate: "Şablonu kaydet",
    templateSaved: "Şablon kaydedildi.",
    templateSaveError: "Şablon kaydedilemedi.",
    templateCreated: "Rapor türü oluşturuldu.",
    templateDeleted: "Rapor türü silindi.",
    deleteSection: "Bölümü sil",
    dragToReorder: "Sürükleyerek sıralayın",
    creatingTemplate: "Şablon oluşturuluyor...",
    loadingTemplates: "Şablonlar yükleniyor...",
    emptyTemplateState: "Soldan bir rapor türü seçin veya yeni bir şablon oluşturun.",
    createFirstTemplate: "İlk şablonu oluştur",
    createFirstTemplatePrompt: "Henüz şablon yok. İlk şablonu oluşturabilir veya şablonsuz özel rapor başlatabilirsiniz.",
    untitledTemplate: "Yeni Şablon",
    untitledTemplateDescription: "Başlangıç için oluşturulan taslak rapor şablonu.",
    starterSectionIntro: "Giriş",
    starterSectionIntroDescription: "Raporun amacı, kapsamı ve temel proje bağlamı.",
    starterSectionAnalysis: "Analiz",
    starterSectionAnalysisDescription: "Toplanan verilerin değerlendirilmesi ve ana bulgular.",
    starterSectionConclusion: "Sonuç",
    starterSectionConclusionDescription: "Özet değerlendirme ve önerilen sonraki adımlar.",
    reportSourcesTitle: "Rapor türü kaynakları",
    reportSourcesDescription: "Her rapor türü için otomatik olarak kullanılacak varsayılan internet kaynaklarını yönetin.",
    defaultSources: "Varsayılan kaynak adresleri",
    defaultSourcesHelp: "Yeni rapor oluşturulduğunda bu adresler otomatik olarak alınır ve rapora bağlanır.",
    addSourceUrl: "Kaynak ekle",
    loadingSources: "Kaynaklar yükleniyor...",
    deleteSource: "Kaynağı sil",
    noDefaultSources: "Bu rapor türü için henüz varsayılan kaynak tanımlanmadı.",
    saveSources: "Kaynakları kaydet",
    reportSourcesSaved: "Kaynaklar kaydedildi.",
    reportSourcesSaveError: "Kaynaklar kaydedilemedi.",
    webResearchEnabled: "AI internet araştırması açık",
    webResearchDisabled: "AI internet araştırması kapalı",
  },
  en: {
    dashboard: "Dashboard", reportIntelligence: "Report Intelligence", reports: "Reports", knowledgeBase: "Knowledge Base",
    seniorConsultant: "Senior Consultant", workspaceSettings: "Workspace settings",
    notifications: "Notifications", aiOperational: "AI services operational",
    reportWorkspace: "Report workspace", greeting: "Good morning",
    dashboardDescription: "Build reliable, source-grounded reports with your team’s knowledge.",
    newReport: "New report", allReports: "All reports", inProgress: "In progress",
    needsReview: "Needs review", completed: "Completed", recentReports: "Recent reports",
    recentReportsDescription: "Continue editing or review generated content.", searchReports: "Search reports...",
    loadingReports: "Loading reports...", progress: "Progress",
    noReports: "No reports yet. Create the first one to get started.", backToDashboard: "Back to dashboard",
    newReportEyebrow: "New report", newReportTitle: "Set up your report workspace",
    newReportDescription: "Provide the project context. Sources and documents can also be added later.",
    reportDetails: "Report details", reportType: "Report type", projectName: "Project name",
    projectPlaceholder: "e.g. North District Development", locationParcel: "Location and parcel",
    city: "City", district: "District", neighborhood: "Neighborhood",
    parcelInfo: "Parcel / block information", parcelPlaceholder: "Block 123, Parcel 45",
    aiSettings: "AI Settings",
    aiSettingsDescription: "When enabled, AI may use configured report sources and trusted web sources.",
    outputPreferences: "Output preferences", outputLanguage: "Report output language",
    desiredLength: "Desired report length", pagesApprox: "Approximately {count} pages",
    createReport: "Create report", creatingWorkspace: "Creating workspace...", whatNext: "What happens next?",
    nextTemplate: "A 10-section professional template is created.", nextConfiguredSources: "Configured report-type sources are fetched automatically.",
    nextFiles: "Upload PDF, DOCX, or TXT source files.", nextGenerate: "Generate and review each section with citations.",
    manualReviewNote: "AI-generated claims without adequate evidence are automatically marked for manual review.",
    planningReport: "Planning & Development Report", feasibilityReport: "Feasibility Report",
    dueDiligenceReport: "Due Diligence Report", marketAssessment: "Market Assessment",
    turkish: "Turkish", english: "English", german: "German", save: "Save", saving: "Saving...",
    allChangesLocal: "All changes local", exportDocx: "Export DOCX", reportProgress: "REPORT PROGRESS",
    sectionsStarted: "{completed} of {total} sections started", reportSections: "Report sections",
    section: "Section", confidence: "confidence", contentEditor: "Content editor", words: "words",
    regenerateSection: "Regenerate section", generateSection: "Generate section",
    editorPlaceholder: "Generate this section with AI or begin writing here...",
    evidenceChecks: "Evidence checks enabled", markApproved: "Mark approved",
    assistant: "Assistant", sources: "Sources", review: "Review", quickActions: "Quick actions",
    formal: "Make more formal", expandEvidence: "Expand with evidence", findMissing: "Find missing information",
    showUnsupported: "Show unsupported claims", convertTable: "Convert into a table",
    officialOnly: "Use only official sources",
    assistantReady: "I’m ready to help with this section. I’ll use the report context and keep unsupported claims visible.",
    applyRevision: "Apply revision", reviewingContext: "Reviewing context...", askSection: "Ask about this section...",
    evidenceLibrary: "Evidence library", evidenceDescription: "Web sources and uploaded files available to AI generation.",
    uploadFiles: "Upload PDF, DOCX, or TXT", extractingText: "Extracting text...",
    officialWebSource: "Official web source", webSource: "Web source",
    configuredSources: "Configured sources",
    manualSources: "Manual sources",
    aiDiscoveredSources: "AI discovered sources",
    aiDiscoveredSource: "AI discovered",
    configuredSource: "Configured source",
    manualSource: "Manual source",
    searchQuery: "Search query",
    indexedChunks: "{count} indexed chunks", noEvidence: "No evidence added yet.",
    unsupportedClaims: "Unsupported claims", missingInformation: "Missing information",
    usedSources: "Used sources", noWarnings: "No active validation warnings for this section.",
    loadingWorkspace: "Loading report workspace...", draft: "Draft", notStarted: "Not started",
    generated: "Generated", approved: "Approved", high: "High", medium: "Medium", low: "Low",
    allowWebResearch: "Allow AI to use web research",
    allowWebResearchHelp: "When enabled, AI may use configured report sources and trusted web sources.",
    settings: "Settings", sourceSettings: "Report sources",
    profile: "Profile",
    workspaceAccount: "Workspace Account",
    profileDescription: "View your personal details, update your username, and change your sign-in password.",
    profileLoading: "Loading profile...",
    personalInformation: "Personal Information",
    personalInformationDescription: "Name and contact details shown across the application.",
    security: "Security",
    securityDescription: "Confirm your current password to update the sign-in password.",
    fullName: "Full name",
    title: "Title",
    email: "Email",
    phone: "Phone",
    username: "Username",
    updated: "Updated",
    currentPassword: "Current password",
    newPassword: "New password",
    confirmNewPassword: "Confirm new password",
    saveProfile: "Save profile",
    savingProfile: "Saving...",
    updatePassword: "Update password",
    updatingPassword: "Updating...",
    profileUpdated: "Profile updated.",
    passwordUpdated: "Password updated.",
    signOut: "Sign out",
    signingOut: "Signing out...",
    signOutError: "Could not sign out.",
    secureWorkspace: "Secure Workspace",
    authentication: "Authentication",
    signIn: "Sign in",
    signingIn: "Signing in...",
    loginTitle: "Sign in to access the report workspace.",
    loginDescription: "Configured API and page routes now require an authenticated session.",
    loginWorkspaceDescription: "This first step closes the app and API surface to unauthorized access. A later phase can add user-level authorization and report ownership.",
    loginFailed: "Sign in failed.",
    loginRequestFailed: "Sign-in request could not be completed.",
    invalidUsernameOrPassword: "Invalid username or password.",
    profileLoadError: "Could not load profile.",
    profileUpdateError: "Could not update profile.",
    passwordUpdateError: "Could not update password.",
    passwordRuleMinLength: "The new password must be at least 8 characters.",
    passwordRuleMatch: "The new password and confirmation must match.",
    passwordRuleCurrent: "The current password must be correct.",
    invalidRequestData: "The submitted data is invalid.",
    passwordConfirmationMismatch: "The password confirmation does not match.",
    currentPasswordIncorrect: "The current password is incorrect.",
    fieldRequired: "This field is required.",
    templateSettings: "Report Templates",
    reportTemplates: "Report Templates",
    reportTemplatesTitle: "Report Templates",
    reportTemplatesDescription: "Manage report types, sections, and default sources in one place.",
    createReportType: "Create report type",
    importTemplateFromDocument: "Create template from document",
    importingTemplate: "Extracting template...",
    templateImported: "Template draft prepared.",
    templateImportError: "Could not create template from document.",
    importedTemplateDraft: "Imported draft",
    importedTemplateNeedsSave: "This template has not been saved yet. Review and save it.",
    deleteReportType: "Delete report type",
    noReportTypes: "No report types yet. Create a new template to get started.",
    generalSettings: "General Settings",
    sectionsTab: "Sections",
    sourcesTab: "Sources",
    aiConfigurationTab: "AI Configuration",
    generalTab: "General Settings",
    sectionTitle: "Section title",
    sectionDescription: "Section description",
    sectionAiPromptLabel: "AI Prompt",
    sectionAiPromptPlaceholder: "Describe the location using official government sources.\nExplain the province, district, neighbouring settlements and regional characteristics.\nWrite in formal planning language.",
    sectionRequiredLabel: "Required",
    sectionEnabledLabel: "Enabled",
    aiConfigGeneralCardTitle: "General",
    aiConfigPromptCardTitle: "AI Prompt",
    aiConfigOutputCardTitle: "Output",
    aiConfigCitationsCardTitle: "Citations",
    aiConfigReportLanguageLabel: "Report Language",
    aiConfigEnableWebResearchLabel: "Enable Web Research",
    aiConfigDefaultPromptLabel: "Default AI Prompt",
    aiConfigDefaultPromptPlaceholder: "Generate a professional zoning report.\nUse official terminology.\nNever invent information.\nUse reliable government sources whenever possible.",
    aiConfigCreativityLabel: "Creativity Level",
    aiConfigRequireCitationsLabel: "Require citations",
    aiConfigReportToneLabel: "Report Tone",
    aiConfigDocumentFormatLabel: "Document Format",
    aiConfigToneFormal: "Formal",
    aiConfigToneTechnical: "Technical",
    aiConfigToneLegal: "Legal",
    aiConfigToneAcademic: "Academic",
    addSection: "Add section",
    noSections: "No sections are configured for this report type yet.",
    sourceName: "Source name",
    sourceDescription: "Source description",
    addSource: "Add source",
    noTemplateSources: "No default sources are configured for this report type yet.",
    createTemplatePlaceholder: "e.g. Zoning Explanation Report",
    templateCreateHint: "If you enter a name, it is created with that name. If left blank, a starter draft template is created automatically.",
    templateDescriptionPlaceholder: "Briefly describe what this report type is used for.",
    saveTemplate: "Save template",
    templateSaved: "Template saved.",
    templateSaveError: "Could not save template.",
    templateCreated: "Report type created.",
    templateDeleted: "Report type deleted.",
    deleteSection: "Delete section",
    dragToReorder: "Drag to reorder",
    creatingTemplate: "Creating template...",
    loadingTemplates: "Loading templates...",
    emptyTemplateState: "Select a report type from the left or create a new template.",
    createFirstTemplate: "Create first template",
    createFirstTemplatePrompt: "No templates exist yet. Create the first template or continue with a custom report without a template.",
    untitledTemplate: "New Template",
    untitledTemplateDescription: "Starter draft report template created for quick setup.",
    starterSectionIntro: "Introduction",
    starterSectionIntroDescription: "Purpose, scope, and core project context of the report.",
    starterSectionAnalysis: "Analysis",
    starterSectionAnalysisDescription: "Evaluation of collected information and key findings.",
    starterSectionConclusion: "Conclusion",
    starterSectionConclusionDescription: "Summary assessment and recommended next steps.",
    reportSourcesTitle: "Report type sources",
    reportSourcesDescription: "Manage the default web sources automatically used for each report type.",
    defaultSources: "Default source URLs",
    defaultSourcesHelp: "These URLs are fetched and attached automatically when a new report is created.",
    addSourceUrl: "Add source", loadingSources: "Loading sources...",
    deleteSource: "Delete source", noDefaultSources: "No default sources are configured for this report type.",
    saveSources: "Save sources", reportSourcesSaved: "Sources saved.",
    reportSourcesSaveError: "Could not save sources.",
    webResearchEnabled: "AI web research enabled",
    webResearchDisabled: "AI web research disabled",
  },
} as const;

type TranslationKey = keyof typeof translations.tr;

type LanguageContextValue = {
  language: UiLanguage;
  setLanguage: (language: UiLanguage) => void;
  t: (key: TranslationKey, variables?: Record<string, string | number>) => string;
};

const LanguageContext = createContext<LanguageContextValue | null>(null);

export function LanguageProvider({ children }: { children: React.ReactNode }) {
  const [language, setLanguageState] = useState<UiLanguage>("tr");

  useEffect(() => {
    const saved = window.localStorage.getItem("ui-language");
    if (saved === "tr" || saved === "en") setLanguageState(saved);
  }, []);

  function setLanguage(next: UiLanguage) {
    setLanguageState(next);
    window.localStorage.setItem("ui-language", next);
    document.documentElement.lang = next;
  }

  const value = useMemo<LanguageContextValue>(() => ({
    language,
    setLanguage,
    t: (key, variables) => {
      let text: string = translations[language][key];
      for (const [name, value] of Object.entries(variables || {})) {
        text = text.replace(`{${name}}`, String(value));
      }
      return text;
    },
  }), [language]);

  return <LanguageContext.Provider value={value}>{children}</LanguageContext.Provider>;
}

export function useLanguage() {
  const context = useContext(LanguageContext);
  if (!context) throw new Error("useLanguage must be used within LanguageProvider.");
  return context;
}

export function LanguageSelector({ compact = false }: { compact?: boolean }) {
  const { language, setLanguage } = useLanguage();
  return (
    <label className="relative">
      <span className="sr-only">Arayüz dili</span>
      <select
        aria-label="Arayüz dili"
        value={language}
        onChange={(event) => setLanguage(event.target.value as UiLanguage)}
        className="rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 text-[10px] font-bold text-slate-600 outline-none"
      >
        <option value="tr">{compact ? "TR" : "Türkçe"}</option>
        <option value="en">{compact ? "EN" : "English"}</option>
      </select>
    </label>
  );
}
